...
object Dijkstra {
  ...
  def shortestPath[Key](lookup: Map[Key, List[(Double, Key)]], fringe: List[Path[Key]], dest: Key, visited: Set[Key]): Path[Key] = fringe match {
    case (dist, path) :: fringe_rest => path match {case key :: path_rest =>
      if (key == dest) (dist, path.reverse)
        ...
        shortestPath(lookup, sorted_fringe, dest, visited + key)
      }
    }
    case Nil => (0d, List())
  }
